...
def db_execute(query, params=(), commit=True, fetch_one=False, fetch_all=False):
    """
    Безопасное выполнение SQL-запросов с блокировкой для многопоточного доступа.
    :param query: SQL запрос
    :param params: Параметры запроса
    :param commit: Коммитить ли изменения
    :param fetch_one: Вернуть одну строку
    :param fetch_all: Вернуть все строки
    :return: Результат запроса или cursor
    """
    with db_lock:
        try:
            cur = conn.cursor()
            ...
            logging.error(f"Database error: {e}, Query: {query[:100]}")
            conn.rollback()
            raise


def db_executemany(query, params_list, commit=True):
    """
    Безопасное выполнение множественных SQL-запросов.
    """
    with db_lock:
        try:
            cur = conn.cursor()
...
def init_db_and_migrate():
    ...
    try:
        ...
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_subscriptions_period ON subscriptions(current_period_month, current_period_year)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_subscriptions_end_ts ON subscriptions(end_ts)"
        )
        cursor.execute(
...
def cmd_check_groups(message):
    ...
    with db_lock:
        cursor.execute(
            "SELECT DISTINCT p.id, p.title, p.group_id, mg.title FROM plans p LEFT JOIN managed_groups mg ON p.group_id = mg.chat_id"
        )
...
@bot.callback_query_handler(
    func=lambda call: call.data and call.data.startswith("approve_payment:")
)
def callback_approve_payment(call):
    ...
    with db_lock:
        ...
        if not payment:
            bot.answer_callback_query(
                call.id, "❌ Заявка не найдена или уже обработана."
            )
            return
    ...
    if success:
        ...
        try:
            ...
            text = (
                f"✅ Ваша заявка на оплату группы '{plan_title}' одобрена!\n\n"
        ...
        try:
            bot.edit_message_caption(
                f"✅ ЗАЯВКА ОДОБРЕНА", call.message.chat.id, call.message.message_id
            )
        except:
            pass
    else:
...
def get_payment_options(user_id, plan_id):
    ...
    has_active_sub, sub_info, message = check_existing_active_subscription(
        user_id, plan_id
    )
                ...
                if new.user and new.user.id == BOT_ID:
                    ...
                    if status in ("administrator", "creator"):
                        ...
                        try:
                            ...
                            logging.exception(f"Error deleting group {chat_id} from DB")
                        for aid in ADMIN_IDS:
                            try:
                                bot.send_message(
                                    aid,
...
@bot.callback_query_handler(
    func=lambda call: call.data and call.data.startswith("renew:")
)
def callback_renew_plan(call):
    ...
    try:
        ...
        if existing_sub:
            ...
            if part_paid == "full":
                bot.answer_callback_query(
                    call.id, "✅ Подписка уже оплачена за этот месяц"
                )
                return

        # Показываем варианты оплаты
        show_plan_full_info(
            call.message.chat.id, user_id, plan_id, show_back_button=True
        )
...
def show_plans(message):
    ...
    if len(rows) == 1:
        ...
        for r in rows:
            ...
            markup.add(
                types.InlineKeyboardButton(
                    f"{title}", callback_data=f"select_plan:{pid}"
                )
            )
...
def show_plan_full_info(chat_id, user_id, plan_id, show_back_button=True):
    ...
    if has_active_sub and sub_info and sub_info[2] == "full":
        ...
        if period_month == current_month and period_year == current_year:
            ...
            text = (
                f"✅ <b>У вас уже есть активная подписка на группу '{plan_title_existing}'!</b>\n\n"
    ...
    if has_active_sub and sub_info and sub_info[2] == "first":
        text += (
            f"⚠️ <b>Внимание:</b> У вас уже оплачена первая часть за этот период!\n\n"
        )
    ...
    if payment_options:
        ...
        for option in payment_options:
            ...
            if (
                has_active_sub
                and sub_info
                and sub_info[2] == "first"
                and option["type"] in ("full", "full_anytime")
            ):
        ...
        markup.add(
            types.InlineKeyboardButton(
                "🎫 Оплатить с промокодом", callback_data=f"enter_promo_main:{plan_id}"
            )
        )
        ...
        if has_active_sub and sub_info and sub_info[2] == "first":
            ...
            text += (
                "💳 <b>Период оплаты второй части:</b> 15-20 числа\n\n"
                "Возвращайтесь в указанные даты!"
            )
...
@bot.callback_query_handler(
    func=lambda call: call.data and call.data.startswith("pay_second_part_from_sub:")
)
def callback_pay_second_part_from_sub(call):
    ...
    try:
        ...
        plan_id, plan_title, part_paid, period_month, period_year, price_cents = (
            subscription
        )
        ...
        text = (
            f"💳 <b>Доплата второй части для группы '{plan_title}'</b>\n\n"
        ...
        for method_id, name, mtype, method_desc, details in payment_methods:
            markup.add(
                types.InlineKeyboardButton(
                    name,
                    callback_data=f"paymethod_second_part:{sub_id}:{method_id}:{payment_type}",
                )
            )
        ...
        markup.add(
            types.InlineKeyboardButton(
                "🎫 Ввести промокод",
                callback_data=f"enter_promo_second:{sub_id}:{payment_type}",
            )
        )
        markup.add(
            types.InlineKeyboardButton(
                "🔙 Назад", callback_data="back_to_my_subscriptions"
            )
        )
...
@bot.callback_query_handler(
    func=lambda call: call.data and call.data.startswith("paymethod_second_part:")
)
...
@bot.callback_query_handler(
    func=lambda call: call.data and call.data.startswith("enter_promo_second:")
)
...
def callback_back_to_my_subscriptions(call):
    ...
    try:
        ...
        logging.exception("Error in callback_back_to_my_subscriptions")
    bot.answer_callback_query(call.id)


@bot.callback_query_handler(
    func=lambda call: call.data and call.data.startswith("enter_promo_main:")
)
...
def show_my_subscription(message):
    ...
    for row in rows:
        ...
        end_date_str = datetime.fromtimestamp(end_ts, LOCAL_TZ).strftime(
            "%d.%m.%Y %H:%M"
        )

        # Формируем текст
        txt = (
            f"🎫 Группа: <b>{title or pid}</b>\n"
        ...
        if needs_renewal:
            ...
            markup.add(
                types.InlineKeyboardButton(
                    "🔄 Продлить подписку", callback_data=f"renew:{pid}"
                )
            )
            ...
            markup.add(
                types.InlineKeyboardButton(
                    "🔗 Получить ссылку", callback_data=f"get_link:{sid}"
                )
            )
        ...
        if part_paid == "first":
            markup.add(
                types.InlineKeyboardButton(
                    "💳 Доплатить вторую часть",
                    callback_data=f"pay_second_part_from_sub:{sid}",
                )
            )
        ...
        if period_month == current_month and period_year == current_year:
            if part_paid == "full":
                return (
                    True,
                    existing_sub,
                    f"У вас уже есть активная подписка на группу '{plan_title}' за текущий месяц!",
                )
            elif part_paid == "first":
                return (
                    True,
                    existing_sub,
                    f"У вас уже оплачена первая часть за группу '{plan_title}'. Вы можете доплатить вторую часть.",
                )
        else:
            ...
            return (
                True,
                existing_sub,
                f"У вас есть активная подписка на группу '{plan_title}', но за другой период.",
            )
...
def callback_select_plan(call):
    try:
        ...
        success = show_plan_full_info(
            call.message.chat.id, user.id, pid, show_back_button=True
        )
    ...
    try:
        ...
        if has_active_sub and sub_info and sub_info[2] == "full":
            ...
            if period_month == current_month and period_year == current_year:
                ...
                text = (
                    f"✅ <b>У вас уже оплачен текущий месяц в группе '{plan_title_existing}'!</b>\n\n"
                ...
                markup.add(
                    types.InlineKeyboardButton(
                        "🔙 Назад к группам", callback_data="back_to_plans"
                    )
                )
                markup.add(
                    types.InlineKeyboardButton(
                        "🎫 Мои подписки", callback_data="show_my_subscriptions"
                    )
                )
        ...
        # Если есть активная подписка с ПЕРВОЙ частью
            ...
            if period_month == current_month and period_year == current_year:
                ...
                if payment_type == "partial":
                    bot.answer_callback_query(
                        call.id, "❌ У вас уже оплачена первая часть"
                    )
                    return
                ...
                # Пользователь пытается купить ПОЛНУЮ
                    ...
                    bot.answer_callback_query(
                        call.id, "⚠️ У вас уже есть частичная оплата"
                    )

                    text = (
                        f"⚠️ <b>У вас уже оплачена первая часть за группу '{plan_title_existing}'</b>\n\n"
                    ...
                    markup.add(
                        types.InlineKeyboardButton(
                            "🔙 Назад", callback_data="back_to_plans"
                        )
                    )
        ...
        for method_id, name, mtype, method_desc, details in payment_methods:
            markup.add(
                types.InlineKeyboardButton(
                    name, callback_data=f"paymethod:{pid}:{method_id}:{payment_type}"
                )
            )
        ...
        markup.add(
            types.InlineKeyboardButton(
                "🎫 Ввести промокод", callback_data=f"enter_promo:{pid}:{payment_type}"
            )
        )
...
@bot.callback_query_handler(
    func=lambda call: call.data and call.data.startswith("buy_full_override:")
)
...
@bot.callback_query_handler(
    func=lambda call: call.data and call.data.startswith("buy_second_part:")
)
...
@bot.callback_query_handler(
    func=lambda call: call.data and call.data.startswith("enter_promo:")
)
...
@bot.message_handler(
    ...
    and user_states[m.from_user.id].get("mode") in ["promo_input", "promo_input_main"]
    and m.text
...
def handle_promo_code_input(message):
    ...
    if message.text.strip() == "❌ Отмена":
        ...
        bot.send_message(
            message.chat.id, "❌ Ввод промокода отменен.", reply_markup=markup
        )

        bot.send_message(
            message.chat.id, "📋 Главное меню:", reply_markup=main_menu(user_id)
        )

        if mode == "promo_input_main":
            # Возвращаемся к просмотру группы
            show_plan_full_info(
                message.chat.id, user_id, state["plan_id"], show_back_button=True
            )
        else:
    ...
    if not promo_data:
        bot.send_message(
            message.chat.id,
            "❌ Промокод не найден. Попробуйте другой или нажмите '❌ Отмена'.",
        )
        return
...
def show_payment_methods_with_promo(chat_id, user_id, state):
        ...
        if mtype == "card":
            ...
            class FakeCall:
                def __init__(self, chat_id):
                    self.message = type(
                        "Message", (), {"chat": type("Chat", (), {"id": chat_id})}
                    )()
                    self.id = "fake_call"
    ...
    text = (
        f"💳 <b>Оплата {payment_type_text} группы '{state['title']}'</b>\n\n"
...
def show_payment_options_with_promo(chat_id, user_id, state):
    ...
    logging.info(
        f"🔍 DEBUG show_payment_options_with_promo: plan_id={plan_id}, user_id={user_id}"
    )
    ...
    if payment_options:
        ...
        if len(payment_methods) == 1:
            ...
            for option in payment_options:
                ...
                if mtype == "card":
                    ...
                    callback_data = (
                        f"buy_with_promo:{option['type']}:{plan_id}:{state['promo_id']}"
                    )
                    markup.add(
                        types.InlineKeyboardButton(
        ...
        if active_type == "second":
            text += (
                "❌ <b>У вас нет активной первой части оплаты для этой группы.</b>\n\n"
            )
        else:
            text += "❌ <b>Сейчас не период оплаты.</b>\n\n"
    ...
    markup.add(
        types.InlineKeyboardButton(
            "🔙 Назад к группе", callback_data=f"select_plan:{plan_id}"
        )
    )
...
def callback_buy_with_promo(call):
    try:
        ...
        if len(parts) < 4:
            logging.error(
                f"🚫 Invalid callback data format: {callback_data}, parts: {parts}"
            )
            bot.answer_callback_query(call.id, "❌ Ошибка в данных.")
            return
        ...
        try:
            ...
            logging.error(
                f"🚫 Invalid IDs in callback: pid={pid_str}, promo_id={promo_id_str}, error: {e}"
            )
            bot.answer_callback_query(call.id, "❌ Неверные данные.")
            return

        logging.info(
            f"🔍 DEBUG Parsed successfully: pid={pid}, payment_type={payment_type}, promo_id={promo_id}"
        )
        ...
        if promo_data:
            ...
            logging.info(
                f"🔍 DEBUG Promo applied: {original_amount} -> {discounted_amount}"
            )
        else:
        ...
        for method_id, name, mtype, method_desc, details in payment_methods:
            callback_data = (
                f"paymethod_promo:{pid}:{method_id}:{payment_type}:{promo_id}"
            )
...
def show_payment_methods(chat_id, user_id, state):
    ...
    for method_id, name, mtype, method_desc, details in payment_methods:
        markup.add(
            types.InlineKeyboardButton(
                name,
                callback_data=f"paymethod:{state['plan_id']}:{method_id}:{state['payment_type']}",
            )
        )
    ...
    markup.add(
        types.InlineKeyboardButton(
            "🎫 Ввести промокод",
            callback_data=f"enter_promo:{state['plan_id']}:{state['payment_type']}",
        )
    )
...
def process_card_payment(
    ...
    logging.info(
        f"🔍 process_card_payment called: pid={pid}, payment_type={payment_type}, group_id={group_id}, renewal_end_ts={renewal_end_ts}"
    )
    ...
    try:
        if payment_type == "renewal":
            ...
            if renewal_end_ts:
                end_date = datetime.fromtimestamp(renewal_end_ts, LOCAL_TZ).strftime(
                    "%d.%m.%Y"
                )
...
def debug_plan_info(plan_id):
    ...
    with db_lock:
        cursor.execute(
            "SELECT id, title, price_cents, group_id FROM plans WHERE id=?", (plan_id,)
        )
        plan = cursor.fetchone()
    if plan:
        logging.info(
            f"🔍 DEBUG Plan {plan_id}: id={plan[0]}, title={plan[1]}, price={plan[2]}, group_id={plan[3]}"
        )
    else:
...
def process_manual_payment_start(
    ...
    if payment_type == "renewal":
        end_date_str = datetime.fromtimestamp(renewal_end_ts, LOCAL_TZ).strftime(
            "%d.%m.%Y %H:%M"
        )
        text = (
            f"💳 <b>Продление подписки на следующий месяц для группы '{title}'</b>\n\n"
    ...
    markup.add(
        types.InlineKeyboardButton(
            "✅ Я оплатил(а)",
            callback_data=f"confirm_paid_renewal:{pid}:{payment_type}:{renewal_end_ts or 0}",
        )
    )
...
@bot.callback_query_handler(
    func=lambda call: call.data and call.data.startswith("confirm_paid_renewal:")
)
...
def callback_paymethod_promo(call):
    ...
    try:
        ...
        logging.info(
            f"🔍 DEBUG paymethod_promo CALLBACK: user={user.id}, pid={pid}, method_id={method_id}, payment_type={payment_type}, promo_id={promo_id}"
        )
        ...
        logging.info(
            f"🔍 DEBUG Calling process_card_payment with: group_id={state['group_id']}"
        )
...
def debug_groups(message):
    ...
    with db_lock:
        cursor.execute(
            "SELECT chat_id, title FROM managed_groups WHERE chat_id = ?",
            (-1002496898299,),
        )
        group = cursor.fetchone()
    if group:
        bot.send_message(
            message.chat.id, f"✅ Группа -1002496898299 найдена: {group[1]}"
        )
    else:
...
def callback_cancel_payment(call):
    ...
    try:
        ...
        logging.debug(f"Could not edit message: {e}")
...
def got_payment(message):
    ...
    if payload.startswith("renewal:"):
        ...
        success, result = activate_subscription(
            user_id, plan_id, "renewal", renewal_end_ts=renewal_end_ts
        )
    ...
    if found:
        ...
        if payload.startswith("renewal:"):
            ...
            txt = (
                f"✅ <b>Спасибо за продление подписки на группу '{plan_title}'!</b>\n\n"
                f"🔗 Ваша новая приватная ссылка для входа в чат (одноразовая):\n{result}\n\n"
                f"⏰ Подписка будет активна с момента окончания текущей."
            )
        elif payment_type == "half_month":
            txt = (
                f"✅ <b>Спасибо за оплату половины месяца в группе '{plan_title}'!</b>\n\n"
                f"🔗 Ваша приватная ссылка для входа в чат (одноразовая):\n{result}\n\n"
                f"⏰ Подписка активна до 5 числа следующего месяца"
            )
        elif payment_type == "partial":
            txt = (
                f"✅ <b>Спасибо за оплату первой части в группе '{plan_title}'!</b>\n\n"
                f"🔗 Ваша приватная ссылка для входа в чат (одноразовая):\n{result}\n\n"
            ...
            # Полная оплата
            txt = (
                f"✅ <b>Спасибо за оплату группы '{plan_title}'!</b>\n\n"
                f"🔗 Ваша приватная ссылка для входа в чат (одноразовая):\n{result}\n\n"
                f"⏰ Подписка активна до 5 числа следующего месяца"
            )
...
def admin_menu(message):
    ...
    markup.row(
        types.KeyboardButton("🎫 Промокоды"), types.KeyboardButton("🔙 Главное меню")
    )
...
def cmd_users(message):
    ...
    for user_id, referred_by, cashback_cents, username, join_date in rows:
        ...
        join_date_str = (
            datetime.fromtimestamp(join_date, timezone.utc).strftime("%Y-%m-%d")
            if join_date
            else "N/A"
        )
...
def cmd_pending_payments(message):
    ...
    for row in rows:
        ...
        text = (
            ...
            f"💳 Тип оплаты: {payment_type_text}\n"
            f"👤 ФИО: {full_name}\n"
            f"⏰ Время заявки: {datetime.fromtimestamp(created_ts, timezone.utc).strftime('%Y-%m-%d %H:%M:%S')} UTC"
        )
...
def admin_create_handler(message):
    ...
    if step == "title":
        ...
        state["step"] = "price"
        bot.send_message(
            message.chat.id, "Шаг 2/6: Укажите цену за месяц (например: 14.99):"
        )
        return
    ...
    if step == "price":
        if not message.text:
            bot.send_message(
                message.chat.id, "❌ Укажите цену текстом, например: 14.99"
            )
            return
        ...
        bot.send_message(
            message.chat.id, "Шаг 3/6: Отправьте описание группы обучения:"
        )
        return
    ...
    if step == "description":
        ...
        if not groups:
            bot.send_message(
                message.chat.id,
                "❌ Нет доступных групп. Сначала добавьте бота в группу.",
            )
        ...
        for chat_id, title, chat_type in groups:
            markup.add(
                types.InlineKeyboardButton(
                    f"{title} ({chat_type})", callback_data=f"select_group:{chat_id}"
                )
            )
        markup.add(
            types.InlineKeyboardButton(
                "⏩ Использовать группу по умолчанию",
                callback_data="select_group:default",
            )
        )

        bot.send_message(
            message.chat.id,
            "Шаг 4/6: Выберите группу для этого тарифа:",
            reply_markup=markup,
        )
        return
    ...
    if step == "media":
        if message.photo:
            ...
            bot.send_message(
                message.chat.id,
                f"✅ Фото добавлено! Всего: {len(state['media_files'])}",
            )
            return
        if message.video:
            ...
            bot.send_message(
                message.chat.id,
                f"✅ Видео добавлено! Всего: {len(state['media_files'])}",
            )
            return
        if message.text:
            ...
            if txt == "⏩ Пропустить медиа":
                ...
                with db_lock:
                    cursor.execute(
                        "INSERT INTO plans (title, price_cents, duration_days, description, group_id, created_ts, is_active) VALUES (?, ?, 30, ?, ?, ?, 1)",
                        (
                            state["title"],
                            state["price_cents"],
                            state["description"],
                ...
                bot.send_message(
                    message.chat.id,
                    "✅ Группа обучения сохранена (без превью).",
            ...
            if txt == "✅ Завершить добавление медиа":
                ...
                if cnt == 0:
                    ...
                    bot.send_message(
                        message.chat.id,
                        "✅ Группа обучения сохранена с 1 превью.",
                        reply_markup=main_menu(uid),
                    )
                else:
                    bot.send_message(
                        message.chat.id,
                        f"✅ Группа обучения сохранена! Использовано первое из {cnt} медиа как превью.",
                        reply_markup=main_menu(uid),
                    )
                return
            bot.send_message(
...
def callback_set_default(call):
    ...
    try:
        ...
        logging.debug(f"Could not edit message in callback_set_default: {e}")
...
def callback_viewmedia(call):
    ...
    try:
        ...
        logging.exception("Error sending media files in callback_view_media")
...
def callback_toggle_payment(call):
    ...
    try:
        ...
        logging.debug(f"Could not edit payment config message: {e}")
...
def callback_list_promos(call):
    ...
    for promo in promos:
        ...
        if expires_ts:
            expires_str = datetime.fromtimestamp(expires_ts, timezone.utc).strftime(
                "%Y-%m-%d %H:%M:%S UTC"
            )
...
def send_payment_notifications():
    ...
    try:
        ...
        # Проверяем триггеры уведомлений (диапазон времени вместо точной минуты)
        notification_schedule = [
            # 1-е число с 10:00 до 10:59 - начало месяца
            (1, 10, "first_of_month", send_first_of_month_notifications),
            # 4-е число с 18:00 до 18:59 - напоминание о дедлайне первой части
            (4, 18, "first_deadline_reminder", send_first_deadline_notifications),
            # 15-е число с 10:00 до 10:59 - вторая часть
            (15, 10, "second_part_start", send_second_part_notifications),
            # 19-е число с 18:00 до 18:59 - напоминание о дедлайне второй части
            (19, 18, "second_deadline_reminder", send_second_deadline_notifications),
        ]

        for day, hour, notification_type, handler_func in notification_schedule:
            if current_day == day and current_hour == hour:
                # Создаем уникальный ключ для предотвращения дубликатов
                notification_key = (
                    f"{current_year}-{current_month:02d}-{notification_type}"
                )

                if notification_key not in sent_notifications[notification_type]:
                    logging.info(
                        f"📢 Sending {notification_type} notifications on {current_day}.{current_month} at {current_hour}:00"
                    )
                    handler_func(now)
                    sent_notifications[notification_type].add(notification_key)
                    logging.info(
                        f"✅ {notification_type} notifications sent and marked"
                    )
...
def send_first_of_month_notifications(now):
    ...
    for user_id, username, plan_title, part_paid in users:
        try:
            ...
            markup.add(
                types.InlineKeyboardButton(
                    "🔄 Продлить подписку", callback_data="renew_subscription"
                )
            )
...
def send_first_deadline_notifications(now):
    ...
    for user_id, username, plan_title, part_paid in users:
        try:
            ...
            markup.add(
                types.InlineKeyboardButton(
                    "🔄 СРОЧНО оплатить", callback_data="renew_subscription"
                )
            )
            ...
            logging.error(
                f"❌ Error sending first deadline notification to user {user_id}: {e}"
            )
...
def send_second_part_notifications(now):
    ...
    for user_id, username, plan_title, price_cents in users:
        try:
            ...
            markup.add(
                types.InlineKeyboardButton(
                    "💳 Оплатить вторую часть", callback_data="pay_second_part"
                )
            )
            ...
            logging.error(
                f"❌ Error sending second part notification to user {user_id}: {e}"
            )
...
def send_second_deadline_notifications(now):
    ...
    for user_id, username, plan_title, price_cents in users:
        try:
            ...
            markup.add(
                types.InlineKeyboardButton(
                    "💳 СРОЧНО оплатить", callback_data="pay_second_part"
                )
            )
            ...
            logging.error(
                f"❌ Error sending second deadline notification to user {user_id}: {e}"
            )
...
def safe_send_message(chat_id, text, **kwargs):
    ...
    if chat_id in last_message_time:
        ...
        if time_since_last < MESSAGE_RATE_LIMIT:
            time.sleep(MESSAGE_RATE_LIMIT - time_since_last)
    ...
    for attempt in range(max_retries):
        try:
            ...
            if e.error_code == 429:  # Too Many Requests
                ...
                logging.warning(
                    f"Rate limit hit for chat {chat_id}, waiting {retry_after}s"
                )
            ...
            logging.exception(f"Error sending message to {chat_id}")
            if attempt == max_retries - 1:
                raise
            time.sleep(2**attempt)
    ...
    while True:
        try:
            ...
            if last_check_date != current_date:
                ...
                logging.info(f"🔄 Notification flags reset for {current_date}")
...
def callback_pay_second_part(call):
    ...
    try:
        ...
        if not subscription:
            bot.answer_callback_query(
                call.id, "❌ Нет подписок, ожидающих вторую часть оплаты"
            )
            return
        ...
        logging.exception("Error in callback_pay_second_part")
        bot.answer_callback_query(call.id, "❌ Ошибка при обработке запроса")


@bot.callback_query_handler(
    func=lambda call: call.data and call.data.startswith("get_link:")
)
...
def callback_show_plans_notification(call):
    ...
    try:
        ...
        logging.exception("Error in callback_show_plans_notification")
        bot.answer_callback_query(call.id, "❌ Ошибка")
    ...
    while True:
        try:
            ...
            if now.day == 6 and now.hour == 0 and last_run_day6 != current_date:
                ...
                logging.info(
                    f"📊 Найдено {len(expired_subs)} подписок для удаления (не оплатили вовсе)"
                )
                ...
                for (
                    ...
                    try:
                        ...
                        if group_id:
                            try:
                                bot.ban_chat_member(
                                    group_id, user_id, until_date=int(time.time()) + 30
                                )
                                logging.info(
                        ...
                        logging.error(
                            f"❌ Ошибка обработки expired подписки {sub_id}: {e}"
                        )
            ...
            # 21-го числа - удаляем тех, кто оплатил только первую часть
                ...
                logging.info(
                    f"📊 Найдено {len(expired_partial_subs)} подписок с частичной оплатой для удаления"
                )
                ...
                for (
                    ...
                    try:
                        ...
                        try:
                            ...
                            logging.info(
                                f"📢 Отправлено уведомление пользователю {username or user_id} о частичной оплате"
                            )
                        except Exception as e:
                            logging.warning(
                                f"❌ Не удалось отправить уведомление пользователю {user_id}: {e}"
                        ...
                        logging.error(
                            f"❌ Ошибка обработки expired частичной подписки {sub_id}: {e}"
                        )
            ...
            # Ежедневно проверяем истекшие подписки (на всякий случай)
                ...
                if expired_subs:
                    logging.info(
                        f"📊 Найдено {len(expired_subs)} подписок с истекшим сроком"
                    )
...
def callback_renew_subscription(call):
    ...
    try:
        ...
        if not subscription:
            bot.answer_callback_query(
                call.id, "❌ У вас нет активных подписок для продления"
            )
            return
        ...
        if (
            ...
            bot.answer_callback_query(
                call.id, "✅ Ваша подписка уже оплачена за этот месяц"
            )
            return
        ...
        logging.exception("Error in callback_renew_subscription")
        bot.answer_callback_query(call.id, "❌ Ошибка при продлении подписки")
...
def cmd_send_test_notifications(message):
    ...
    for user_id in test_users:
        try:
            ...
            bot.send_message(
                message.chat.id, f"❌ Ошибка отправки пользователю {user_id}: {e}"
            )

    bot.send_message(
        message.chat.id,
        f"✅ Тестовые уведомления отправлены {len(test_users)} пользователям",
    )
...
def start_bot_with_restart():
    ...
    while True:
        try:
            ...
            restart_attempts += 1
            logging.exception(
                f"❌ Polling crashed! Attempt {restart_attempts}/{max_restart_attempts}"
            )
